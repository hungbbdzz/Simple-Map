...
public class MapRenderer {
     ...
     * @param drawPlayer  Whether to render the player marker at the player's actual position
     */
    public void drawMap(GuiGraphics guiGraphics, int viewportX, int viewportY, int width, int height,
        ...
        // Expand bounds by sqrt(2) (1.415x) when rotated to prevent region clipping at corners
        ...
        // 6. Draw each visible region using raw GL to guarantee nearest-neighbor (no blur)
        for (int rz = minRegionZ; rz <= maxRegionZ; rz++) {
            for (int rx = minRegionX; rx <= maxRegionX; rx++) {
                ...
                if (texture != null) {
                    ...
                    // It operates on the currently bound texture which setShaderTexture establishes.
                    RenderSystem.setShaderTexture(0, texture);
                    RenderSystem.texParameter(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_NEAREST);
                    RenderSystem.texParameter(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_NEAREST);

                    // Position in world space: (rx * 512, rz * 512)
                    // Draw with a size of 512 to match actual world coordinates and prevent coordinates drift.
                    guiGraphics.blit(
                        texture,
                        rx * 512, rz * 512,
                        512, 512,
        ...
        if (MapConfig.waypointsVisible) {
            ...
            for (WaypointManager.Waypoint wp : waypoints) {
                ...
                if (!isMinimap) {
                    // Waypoint width in world coordinates is ~1.28 * waypointScale. So hover radius is ~0.64 * waypointScale (increased slightly to 0.8 for easier hovering)
                    double hoverRadius = 0.8 * MapConfig.waypointScale;
                    isHovered = Math.abs(mouseWorldX - wp.x) <= hoverRadius && Math.abs(mouseWorldZ - wp.z) <= hoverRadius;
                }
        ...
        if (MapConfig.pinActive) {
            if (MapConfig.pinWorldX >= minX && MapConfig.pinWorldX <= maxX &&
                ...
                net.minecraft.resources.ResourceLocation redX =
                        net.minecraft.resources.ResourceLocation.fromNamespaceAndPath("minecraft", "textures/map/decorations/red_x.png");
        ...
        if (drawPlayer) {
            double playerX = net.minecraft.util.Mth.lerp(partialTick, mc.player.xo, mc.player.getX());
            double playerZ = net.minecraft.util.Mth.lerp(partialTick, mc.player.zo, mc.player.getZ());
            float playerYaw = net.minecraft.util.Mth.rotLerp(partialTick, mc.player.yRotO, mc.player.getYRot()) + 180.0f;
     ...
     */
    private void drawWaypointMarker(GuiGraphics guiGraphics, WaypointManager.Waypoint wp, float mapScale, boolean isMinimap, boolean isHovered) {
        Minecraft mc = Minecraft.getInstance();
        PoseStack poseStack = guiGraphics.pose();
        poseStack.pushPose();
        ...
        // Determine item ID (map old preset iconTypes to item textures for backward compatibility)
        String itemID = wp.iconItem;
        if (wp.iconType >= 0) {
            if (wp.iconType == 0) itemID = "minecraft:red_dye";
            else if (wp.iconType == 1) itemID = "minecraft:red_bed";
            else if (wp.iconType == 2) itemID = "minecraft:target";
            else itemID = "minecraft:nether_star";
        }
        ...
        try {
            ...
            if (item != null && item != net.minecraft.world.item.Items.AIR) {
                ...
                guiGraphics.renderFakeItem(new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.COMPASS), -8, -8);
            }
        } catch (Exception e) {
            guiGraphics.renderFakeItem(new net.minecraft.world.item.ItemStack(net.minecraft.world.item.Items.COMPASS), -8, -8);
        }
    ...
    private void drawPlayerMarker(GuiGraphics guiGraphics, double worldX, double worldZ, float yaw, float mapScale) {
        ...
        if (MapConfig.playerMarkerMode == 1) {
            ...
            // Mode 0: DEFAULT (Triangle/Chevron pointer + Player skin head)
            ...
            // 3. Draw Direction Pointer (small chevron pointer at the top of the head)
            poseStack.pushPose();
            poseStack.translate(0, -5, 1);
            drawDirectionalPointer(poseStack, pointerColor, 0.6f);
            poseStack.popPose();
        }
